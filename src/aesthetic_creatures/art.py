...
def draw_gradient_line(
    ...
    while True:
        t = pos / length
        color = get_interpolated_color(t, palette)
        draw_disk(img, x0, y0, thickness, color)
        if x0 == x1 and y0 == y1:
            break
        e2 = 2 * err
...
def get_body_trajectory(data: np.lib.npyio.NpzFile) -> Tuple[np.ndarray, List[str]]:
    body_parts = []
    trajectory = None
    
    priority_keys = ["primary_com", "torso_com", "hand_com", "foot_com", "tip_com"]
    ...
    if trajectory is None:
        raise ValueError("No body trajectory found in rollout data.")
    
    return trajectory, body_parts
...
def make_trail_art(
    ...
    for i in range(len(xy)):
        ...
        for j in range(start + 1, i + 1):
            ...
            draw_gradient_line(img, int(p0[0]), int(p0[1]), int(p1[0]), int(p1[1]), pal, thickness=max(1, int(2 + 3 * t)))
...
def make_art_video(
    ...
    trajectory, body_parts = get_body_trajectory(data)
    rewards = data.get("rewards")
    actions = data.get("actions")
    
    print(f"Generating art for {body_parts[0] if body_parts else 'unknown'} with {len(trajectory)} frames")
